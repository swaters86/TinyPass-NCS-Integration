...
// Do nothing, so the TinyPass button will not display because access has been granted to the visitor
  ...
  $request = new TPPurchaseRequest($offer);
 	...
 	$buttonHTML = $request->setCallback("refreshPage")->generateTag();
 	
	$SOSE->Echo('You have no more views left. To read more <strong>Premium Articles</strong>, click the button below to purchase site-wide access. <br>');
  
  $SOSE->Echo($buttonHTML);
